 ...
 * pipe count. The result depends on all of these parameters together.
 */
struct igt_modeset_intent {
    ...
    int bpc;                 /* Bits per component (0 = default) */
    bool dsc;                /* Whether DSC will be enabled */
...
{
    /* Query from device info / debugfs. Platform-specific values:
     * Pre-MTL: 5120, MTL+: 5120, LNL+: may differ.
     * Returns 0 if not determinable. */
    return intel_display_ver(intel_get_drm_devid(fd)) >= 14 ? 5120 : 5120;
}
                               ...
                               const struct igt_modeset_intent *intent)
{
    ...
    /* Check width-based joiner requirement */
    if (max_pipe_width > 0) {
    ...
    /* Check dotclock-based joiner requirement */
    if (max_dotclock <= 0)
        return 1;

    effective_clock = intent->mode.clock;

    /* DSC reduces effective bandwidth */
    if (intent->dsc)
        effective_clock = effective_clock / 2;

    /* Higher BPC increases bandwidth */
    if (intent->bpc > 8)
...
{
    /*
     * NOTE: This runs from IGT's exit-handler machinery, which may
     * be invoked from signal context. We keep the handler minimal:
     * just write the saved values back. The igt_debugfs_simple_write()
     * calls do filesystem I/O which is technically not async-signal-safe,
     * but IGT's existing exit handlers already do similar I/O (e.g.,
     * igt_cleanup_aperture_trashers, connector state reset), so this
     * is consistent with established IGT practice.
     *
     * For the normal-exit path (igt_assert/igt_require → longjmp →
     * process exit), this is fully safe because signal context is not
     * involved.
     */
...
typedef struct {
    ...
    igt_output_t *connected[IGT_MAX_PIPES];
    int n_connected;
                                  ...
                                  int start_slot)
{
    ...
    /* All slots filled. Verify pipe allocation. */
    uint32_t dummy = 0;
 ...
 *
 * Returns true if backtracking succeeded (caller should retry
 * _find_combo_from_slot from the advanced slot).
 * Returns false if all combos are exhausted.
 */
                              ...
                              igt_output_t **outputs)
{
    ...
    while (true) {
        ...
        /* Find the slot that was advanced by backtracking */
        int advanced_slot = 0;
        for (int s = iter->n_slots - 1; s >= 0; s--) {
            if (iter->cursor[s] > 0 || outputs[s] == NULL) {
                advanced_slot = s;
                break;
            }
        }
...
4. **Pipe feasibility:** Each complete combo is validated through
   `igt_allocate_pipes()` before being yielded. If pipe allocation fails,
   the iterator backtracks to try the next combo.
...
| P06 | `lib/igt_kms: add igt_allocate_pipes()` | Allocator with priority sorting |
